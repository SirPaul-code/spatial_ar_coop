...
object PeerProtocol {
    ...
    fun write(output: OutputStream, message: WireMessage) {
        ...
        DataOutputStream(payload).use { out ->
            when (message) {
                is WireMessage.Hello -> { out.writeUTF(message.username.take(48)); out.writeUTF(message.deviceModel.take(64)) }
                is WireMessage.Frame -> writeFrame(out, message.frame)
                is WireMessage.Poi -> {
                    out.writeLong(message.id); out.writeUTF(message.owner.take(48))
                    repeat(3) { out.writeFloat(message.pointWorld.getOrElse(it) { 0f }) }
                    out.writeLong(message.createdAtMs)
                }
                WireMessage.ClearPoi -> Unit
                is WireMessage.Range -> { out.writeFloat(message.distanceM); out.writeFloat(message.stdDevM); out.writeInt(message.samples) }
                is WireMessage.Quality -> { out.writeFloat(message.confidence); out.writeInt(message.stableCount); out.writeBoolean(message.ready) }
            }
        }
        ...
        out.writeInt(MAGIC); out.writeByte(VERSION); out.writeByte(type); out.writeInt(bytes.size); out.write(bytes); out.flush()
    }
    ...
    fun read(input: InputStream): WireMessage? {
        ...
        val payload = ByteArray(size); source.readFully(payload)
        DataInputStream(ByteArrayInputStream(payload)).use { data ->
            return when (type) {
                ...
                T_POI -> WireMessage.Poi(data.readLong(), data.readUTF(), floatArrayOf(data.readFloat(), data.readFloat(), data.readFloat()), data.readLong())
    ...
    private fun writeFrame(out: DataOutputStream, frame: CapturedFrame) {
        ...
        out.writeFloat(k.fx); out.writeFloat(k.fy); out.writeFloat(k.cx); out.writeFloat(k.cy); out.writeInt(k.width); out.writeInt(k.height)
    ...
    private fun readFrame(input: DataInputStream): CapturedFrame {
        ...
        val intrinsics = IntrinsicsPacket(input.readFloat(), input.readFloat(), input.readFloat(), input.readFloat(), input.readInt(), input.readInt())
        val jpegSize = input.readInt(); require(jpegSize in 1..MAX_PAYLOAD) { "invalid jpeg size $jpegSize" }
        val jpeg = ByteArray(jpegSize); input.readFully(jpeg)
        val count = input.readInt(); require(count in 0..2500) { "invalid point count $count" }
        val points = ArrayList<FloatArray>(count)
        repeat(count) { points += FloatArray(5) { input.readFloat() } }
        return CapturedFrame(timestampNs, PosePacket(t, q), intrinsics, Base64.encodeToString(jpeg, Base64.NO_WRAP), points)
    }
}
