...
object MetricSupportSampler {
    ...
    fun pointAtCpuPixel(frame: Frame, camera: Camera, u: Float, v: Float): FloatArray? {
        return try {
            frame.acquireDepthImage16Bits().use { image ->
                ...
                val x = (tex[0] * image.width).toInt().coerceIn(0, image.width - 1)
                val y = (tex[1] * image.height).toInt().coerceIn(0, image.height - 1)
                val mm = depthMm(image, x, y)
                if (mm <= 0) return null
                unprojectToWorld(camera, u, v, mm / 1000f)
            }
            ...
            frame.acquireDepthImage16Bits().use { image ->
                ...
                while (y < image.height) {
                    ...
                    while (x < image.width) {
                        val mm = depthMm(image, x, y)
                        if (mm in 150..65000) {
                            texList.add((x + 0.5f) / image.width.toFloat())
                            texList.add((y + 0.5f) / image.height.toFloat())
                            depths.add(mm)
                        }
                        x += step
                    }
                ...
                for (i in depths.indices) {
                    val u = cpu[i * 2]
                    val v = cpu[i * 2 + 1]
                    if (!u.isFinite() || !v.isFinite() || u < 0 || v < 0) continue
                    val world = unprojectToWorld(camera, u, v, depths[i] / 1000f)
                    out.add(floatArrayOf(u, v, world[0], world[1], world[2]))
                }
                out
            }
        } catch (_: NotYetAvailableException) {
        ...
        return try {
            frame.acquirePointCloud().use { cloud ->
                ...
                for (i in 0 until count step stride) {
                    ...
                    val z = -ca[2]
                    if (z <= 0.05f) continue
                    val u = f[0] * ca[0] / z + pp[0]
                    val v = f[1] * (-ca[1]) / z + pp[1]
                    if (u !in 0f..dims[0].toFloat() || v !in 0f..dims[1].toFloat()) continue
                    out.add(floatArrayOf(u, v, world[0], world[1], world[2]))
    ...
    private fun depthMm(image: Image, x: Int, y: Int): Int {
        ...
        val index = x * plane.pixelStride + y * plane.rowStride
    ...
    private fun unprojectToWorld(camera: Camera, u: Float, v: Float, depthM: Float): FloatArray {
        ...
        val arCameraPoint = floatArrayOf(xCv, -yCv, -depthM)
        return camera.pose.transformPoint(arCameraPoint)
    }
}
