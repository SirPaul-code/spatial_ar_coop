...
1. `docs/AGENT_CONTEXT_CURRENT.md` — authoritative current runtime state, latest physical regression, safety invariants, exact alignment flow, and acceptance tests.
2. `docs/AGENT_CONTEXT.md` — older long-form architecture/history. Treat `AGENT_CONTEXT_CURRENT.md` as authoritative when they disagree.
...
- After any substantial architecture or physical-test finding, update `docs/AGENT_CONTEXT_CURRENT.md`.

## Current stabilization baseline

Runtime code immediately before the current docs refresh is `3d21b53988103bbb1e2e7ee6a9d1b49dda707006` plus its ancestor false-lock hardening series. The key physical regression was: sender-local target correct at the thermostat, receiver rendered the same target about 21.7–22 m away while UI said `LOCKED`. The current runtime was specifically reworked so this class of false lock is rejected or revoked rather than trusted.

Always inspect current branch HEAD, recent commits, CI, and `latest-dev` before continuing; docs-only commits make branch HEAD newer than the runtime-code SHA above.
