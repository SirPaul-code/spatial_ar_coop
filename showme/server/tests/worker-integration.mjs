...
const secret='test-owner-activation-only-not-production';
const mf=new Miniflare({modules:true,scriptPath:root+'src/worker.mjs',compatibilityDate:'2026-08-06',
  durableObjects:{ROOMS:{className:'ShowMeRoom',useSQLite:true}},bindings:{ROOM_CREATE_KEY:secret,MAX_ROOMS_PER_DAY:'3',SESSION_SECONDS:'300'},
  serviceBindings:{ASSETS:async request=>{
    const path=new URL(request.url).pathname;const allowed=['index.html','setup.html','welcome.html','app.js','style.css','live-video.mjs','geometry.mjs','remote-session.mjs','control-rpc.mjs','setup.mjs'];
...
const post=(path,token,body={})=>mf.dispatchFetch(base+path,{method:'POST',headers:{Authorization:`Bearer ${token}`,'content-type':'application/json'},body:JSON.stringify(body)});
...
try{
  ...
  const created=await (await post('/api/rooms',secret,{name:'Test owner'})).json();assert.equal(created.ok,true);
  const room=created.roomId,guestCap=new URL(created.inviteUrl).hash.slice(1);assert.notEqual(guestCap,created.hostToken);
  ...
  // No missing TURN credentials are silently disguised as reliable Internet connectivity.
  ...
  const extra=await mf.dispatchFetch(`${base}/api/rooms/${room}/ws/guest`,{headers:{Upgrade:'websocket',Authorization:`Bearer ${guestCap}`}});assert.equal(extra.status,409);
  guest.socket.send(JSON.stringify({type:'draw',id:'request003',jpeg:'not media signaling'}));await guest.next('error');
  ...
  assert.equal((await post('/api/rooms',secret)).status,200);assert.equal((await post('/api/rooms',secret)).status,200);
  assert.equal((await post('/api/rooms',secret)).status,429);
  const denied=await mf.dispatchFetch(base+'/api/health',{headers:{Origin:'https://untrusted.test'}});assert.equal(denied.status,403);
  console.log('Worker integration passed: SQLite DO, HTTPS routing, roles/capabilities, owner approval, SDP exchange, no media relay over signaling, room quota and revocation.');
